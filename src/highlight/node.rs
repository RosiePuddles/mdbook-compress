	...
	for name in names {
		if name.starts_with("language-") {
			match std::process::Command::new("node")
				.arg("-e")
				.arg(format!("{};console.log(!!hljs.getLanguage({:?}))", hl, &name[9..name.len()]))
				.output() {
				Ok(out) => {
					let stdout = String::from_utf8(out.stdout).unwrap();
					if out.status.code() != Some(0) {
						println!("Error in getting highlight_.js languages! {}", String::from_utf8(out.stderr).unwrap());
						std::process::exit(1)
					}
					if stdout.trim() == "true" { return Some(name[9..name.len()].to_string()) }
				}
				Err(e) => {
...
pub fn highlight(classes: HashSet<String>, hl: &String, src: String) -> LinearLayout {
	if let Some(lang_name) = check_language(classes, hl) {
		...
		let colour_map = StyleElement::Parent {default: Style::new(), children: BTreeMap::from_iter([
